    ...
    (async () => {
      try {
        const [response] = await Promise.all([
          request<{
            pageBy: { avisodeprivacidad: { content: string; ingles: boolean } };
          }>(ENDPOINT, QUERY_AVISO),
        ]);

        if (!active) return;

        const edges = response?.pageBy?.avisodeprivacidad.content ?? contents;
        const ingles = response?.pageBy?.avisodeprivacidad?.ingles ?? false;
